...
//TERMOSTATO
    ...
    getTemperature: function(callback) {
		...
		/*
		//invia_frame('*#4*' + chi +'##')
		var frame = cancelletto('*#4*' + this.ref +'##')
		var url = this.status_url + "/frame/" + frame
	...
	if (typeof context === 'undefined'){
		...
		if (parseFloat(setpoint_da_convertire) > 99) {
					...
					//this.log('*#4*#' + this.ref + '*#14*' + setpoint_da_convertire +'*3##');  //*#4*#2*#14*0195*1##
				} else {
		...
		//termoset/:setpoint/:where
		var url = this.status_url + "/termoset/" + setpoint_da_convertire + "/" + this.ref
		//this.log(url)
		httpRequest(url, 'GET', function(error, response, body) {
					if (error) {
		...
		setTimeout(	function(){
					...
					//.getValue(undefined);
					this.ThermostatService.getCharacteristic(MyHome2Platform.TermoMode)
					.getValue(undefined);
 ...
 //---------------
	...
	*/				
        ...
        case "Thermostat": {
			...
			ThermostatService.addCharacteristic(MyHome2Platform.TermoMode)
				.on('get', this.getTermoMode.bind(this));

			ThermostatService.addCharacteristic(MyHome2Platform.TermoNOW);
			
			
			// Register a listener for event changes
                eventEmitter.on(this.config.type + ":" + this.id + ":termo_now", function(value) {
					//var position = parseInt(value,10);
					var position = parseFloat(value);
					//this.ThermostatService.setCharacteristic(MyHome2Platform.TermoNOW , "" + value + "\xB0" + "C");
					//this.ThermostatService.setCharacteristic(Characteristic.CurrentTemperature, position);
					this.ThermostatService.getCharacteristic(MyHome2Platform.TermoNOW).setValue("" + value + "\xB0" + "C")
					this.ThermostatService.getCharacteristic(Characteristic.CurrentTemperature).setValue(position);
					
                }.bind(this));

			//this.ThermostatService.setCharacteristic(MyHome2Platform.TermoNOW , ""+value + "\xB0" + "C");
				// Register a listener for event changes
                eventEmitter.on(this.config.type + ":" + this.id + ":termo_setpoint_corrected", function(value) {
					//var position = parseInt(value,10);
					var position = parseFloat(value);
					this.ThermostatService.getCharacteristic(Characteristic.TargetTemperature).setValue(position);
					//this.ThermostatService.getCharacteristic(Characteristic.TargetTemperature).getValue(undefined)
					
                }.bind(this));
							
			// Register a listener for event changes
                eventEmitter.on(this.config.type + ":" + this.id + ":termo_status", function(value) {
					var position = parseInt(value,10);
					//this.ThermostatService.setCharacteristic(Characteristic.CurrentHeatingCoolingState, position);
					//this.ThermostatService.getCharacteristic(Characteristic.CurrentHeatingCoolingState).setValue(position);
					this.ThermostatService.getCharacteristic(Characteristic.CurrentHeatingCoolingState).getValue(undefined)
					
                }.bind(this));	
				
			// Register a listener for event changes
                eventEmitter.on(this.config.type + ":" + this.id + ":termo_tutto", function(value) {
					//Automatic Heating|3|20.5
					var valori = value.split("|")
					var position = parseFloat(valori[2]);
					var stato = parseInt(valori[1],10);
					this.ThermostatService.getCharacteristic(Characteristic.TargetTemperature).setValue(position);
					this.ThermostatService.getCharacteristic(Characteristic.CurrentHeatingCoolingState).getValue(undefined);
					//this.ThermostatService.getCharacteristic(Characteristic.CurrentHeatingCoolingState).setValue(stato);
					this.ThermostatService.getCharacteristic(MyHome2Platform.TermoMode).setValue(valori[0]);
					
					this.ThermostatService.getCharacteristic(Characteristic.CurrentTemperature).setValue(parseFloat(valori[3]));
					this.ThermostatService.getCharacteristic(MyHome2Platform.TermoNOW).setValue("" + valori[3] + "\xB0" + "C")
